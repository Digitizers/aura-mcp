...
2. **Wire an MCP client** — point it at the gateway with the token:
   ...
   **Zero-config alternative:** the repo commits a placeholder-only [`.mcp.json`](.mcp.json)
   ...
   connection authenticates on its own. While the variable is unset the header goes out with
   an empty bearer, so the connection fails with a 401 the client reports as
   `AUTH_HEADER_REJECTED` — set the var to bring it up. Real tokens never go into the file —
